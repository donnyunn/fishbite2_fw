...
#define GATTS_DESCR_UUID_TEST_C     0x3333
...
static void (*ble_demo_sensitivityChanged)(int8_t) = NULL;
static void (*ble_demo_brightnessChanged)(int8_t) = NULL;
static void invokeSensitivityChanged(uint8_t val)
{
    if (ble_demo_sensitivityChanged != NULL)
    {
        (*ble_demo_sensitivityChanged)(val);   
    }
}
static void invokeBrightnessChanged(uint8_t val)
{
    if (ble_demo_brightnessChanged != NULL)
    {
        (*ble_demo_brightnessChanged)(val);
    }
}
...
static void gatts_profile_a_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param) {
    ...
    case ESP_GATTS_WRITE_EVT: {
        ...
        if (!param->write.is_prep){
            ...
            invokeSensitivityChanged(param->write.value[0]);
...
static void gatts_profile_b_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param) {
    ...
    case ESP_GATTS_WRITE_EVT: {
        ...
        if (!param->write.is_prep){
            ...
            invokeBrightnessChanged(param->write.value[0]);
...
void ble_sendIndication(int ID, uint8_t* val)
{
    ...
    {
        ESP_LOGI(GATTS_TAG, "Send indication");
        err =  esp_ble_gatts_send_indicate(
        gl_profile_tab[ID].gatts_if,
        gl_profile_tab[ID].conn_id,
...
{
    ble_demo_sensitivityChanged = fnPtr;
}

void ble_set_onBrightnessChanged(void(*fnPtr))
{
    ble_demo_brightnessChanged = fnPtr;
}
